...
void Index_Core_Fast::all_query_index(uint32_t index_hash_key,std::vector<uint32_t> &query_out)
{
	if (index_hash_key <= 0) {return;}

	// need all data
	{// enter big lock space
		AUTO_LOCK auto_lock(&index_map_key_lock,false);
#ifdef _USE_HASH_
		std::tr1::unordered_map<uint32_t,index_hash_value>::iterator it = index_map.find(index_hash_key);
#else
		std::map<uint32_t,index_hash_value>::iterator it = index_map.find(index_hash_key);
#endif
		if (it != index_map.end())
...
void Index_Core_Fast::cross_query_index(uint32_t index_hash_key,std::vector<uint32_t> &query_in,std::vector<uint32_t> &query_out)
{
	if (index_hash_key <= 0 || query_in.size() <= 0) {return;}
...
bool Index_Core_Fast::insert_index(uint32_t index_hash_key,uint32_t insert_in_value)
{
	{ //enter index_update lock space
		...
		if (index_hash_key <= 0 || insert_in_value <= 0)
		{return false;}

		index_node *p_node_last = 0;
#ifdef _USE_HASH_
		std::tr1::unordered_map<uint32_t,index_hash_value>::iterator it;
#else
		std::map<uint32_t,index_hash_value>::iterator it;
#endif
		{// enter big lock space
			AUTO_LOCK auto_lock(&index_map_key_lock,false);
		...
		if (insert_statue == 2)
		{
			...
			{// enter big lock space
				...
				index_map.insert ( std::pair<uint32_t,index_hash_value>(index_hash_key,*p_hash_value) );
				_hash_now_num++;	
			}
			return true;
...
bool Index_Core_Fast::delete_index(uint32_t index_hash_key,uint32_t delete_in_value)
{
	{ //enter index_update lock space
		AutoLock_Mutex auto_lock0(&index_update_lock);

		if (index_hash_key <= 0 || delete_in_value <= 0)
		{return false;}
...
bool Index_Core_Fast::clear_index(uint32_t index_hash_key)
{
	{ //enter index_update lock space
		AutoLock_Mutex auto_lock0(&index_update_lock);
...
bool Index_Core_Fast::shrink_index(uint32_t index_hash_key,bool adapt_mem)
{
	{ //enter index_update lock space
		AutoLock_Mutex auto_lock0(&index_update_lock);
		...
		if (index_hash_key <= 0) {return false;}
...
//		std::vector<uint32_t> &query_out = *(heap_vec.m_p_vec);
		...
		{// enter big lock space
			...
			if (it != index_map.end())
			{
				...
				index_map.erase(it);
				if (!empty_hash_key)
				{index_map.insert ( std::pair<uint32_t,index_hash_value>(index_hash_key,new_one_hash_value) );}
			}
		}
...
index_hash_value Index_Core_Fast::find_index(uint32_t index_hash_key)
{
...
void Index_Core_Fast::clear_all_index()
{
	{ //enter index_update lock space
		AutoLock_Mutex auto_lock0(&index_update_lock);
		...
		{// enter big lock space
			AUTO_LOCK auto_lock(&index_map_key_lock,true);
#ifdef _USE_HASH_
			for (std::tr1::unordered_map<uint32_t,index_hash_value>::iterator it = index_map.begin(); it!=index_map.end(); ++it)
#else
				for (std::map<uint32_t,index_hash_value>::iterator it = index_map.begin(); it!=index_map.end(); ++it)
#endif
				{
					index_hash_value one_hash_value = {0};//init 0
					one_hash_value = it->second;
