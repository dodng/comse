...
int get_rela_score_2(std::vector<std::string> & term_list,std::set<uint32_t> &term4se_set,void *p_ptr)
{
	if (0 == p_ptr || term_list.size() <= 0 || term4se_set.size() <= 0) {return -1;}
	Search_Engine_Fast *se_fast_p = (Search_Engine_Fast *)p_ptr;
	int left_len = 0;
	int right_len = 0;
	int and_len = 0;

	std::set<uint32_t>::iterator term_hash_it;

	for (int i = 0 ; i < term_list.size();i++)
	{
		left_len += term_list[i].length();

		term_hash_it = term4se_set.find( se_fast_p->query_term(term_list[i]) );
	...
	for (std::set<uint32_t>::iterator it = term4se_set.begin(); it != term4se_set.end(); ++it)
	{
		right_len += se_fast_p->query_term(*it).length();
	}
	int min_len = left_len > right_len ? right_len : left_len;
	return (int)((and_len * 100.0f) / (min_len));
}

int get_rela_score_2(std::set<uint32_t> &term4se_set_1,std::set<uint32_t> &term4se_set_2,void *p_ptr)
{
	
	if (0 == p_ptr || term4se_set_1.size() <= 0 || term4se_set_2.size() <= 0) {return -1;}
	Search_Engine_Fast *se_fast_p = (Search_Engine_Fast *)p_ptr;
	...
	for (std::set<uint32_t>::iterator it = term4se_set_1.begin(); it != term4se_set_1.end(); ++it)
	{
		left_len += se_fast_p->query_term(*it).length();

		term_hash_it = term4se_set_2.find(*it);
		if (term_hash_it != term4se_set_2.end()) 
		{
			and_len += se_fast_p->query_term(*it).length();
		}
	}

	for (std::set<uint32_t>::iterator it = term4se_set_2.begin(); it != term4se_set_2.end(); ++it)
	{
		right_len += se_fast_p->query_term(*it).length();
	}
	int min_len = left_len > right_len ? right_len : left_len;
...
int policy_compute_score(std::string &query,std::vector<std::string> & term_list,std::vector<float> & term_list_score,std::vector<uint32_t> & term_list_int,Json::Value & query_json,info_storage & one_info,int search_mode)
{
	...
	if (search_mode == or_mode || search_mode == rela_mode)
	{
		...
		if ( one_info.term4se_set.size() > 0)
		{
			...
			for (int i = 0 ; i < term_list_int.size();i++)
			{
...
bool json_2_info(Json::Value & json,std::string & json_show_str,info_storage & info_stg,void *p_ptr)
{
	...
	{
		info_stg.term4se_set.insert( se_fast_p->insert_term(json["cmd_info"]["term4se"][i].asString()) );
	}
...
bool info_2_json(info_storage & info_stg, std::string & json_output_str,void *p_ptr)
{
	if (0 == p_ptr) {return false;}
	Search_Engine_Fast *se_fast_p = (Search_Engine_Fast *)p_ptr;
	...
	for (std::set<uint32_t>::iterator it = info_stg.term4se_set.begin(); it != info_stg.term4se_set.end() ;it++)
	{
		...
		{json_output_str += "\"" + str_escape( se_fast_p->query_term(*it) ) + "\"";}
		else
		{json_output_str += ",\"" + str_escape( se_fast_p->query_term(*it) ) + "\"";}
	}
...
bool Search_Engine_Fast::add(std::vector<std::string> & term_list,Json::Value & one_info)
{
	bool ret = false;
	...
	//add index
	{
		...
		for (int i = 0 ;i < term_list.size(); i++)
		{
			if( _index_core.insert_index(query_term(term_list[i]),index_num))
			{ret = true;}

			//check if need shrink
			index_hash_value i_hash_value = _index_core.find_index(query_term(term_list[i]));
					...
					(i_hash_value.use_data_num / i_hash_value.sum_node_num) < DEFAULT_ADD_NEED_SHRINK_AVG) 
			{
				...
				_index_core.shrink_index(query_term(term_list[i]));
				i_hash_value = _index_core.find_index(query_term(term_list[i]));
...
bool Search_Engine_Fast::del(std::vector<std::string> & term_list,Json::Value & one_info)
{
	bool ret = false;
	uint32_t index_num = 0;
	//check if validate
	...
	//del index
	{
		...
		for (int i = 0 ;i < term_list.size(); i++)
		{
			if( _index_core.delete_index(query_term(term_list[i]),index_num))
			{ret = true;}
...
bool Search_Engine_Fast::search(std::vector<std::string> & in_term_list,
		std::string & in_query,
...
bool Search_Engine_Fast::dump_to_file()
{
	std::ofstream os;  
	...
	{
		{
			...
			for (std::tr1::unordered_map<uint32_t,info_storage>::iterator it = _info_dict.begin(); it != _info_dict.end(); ++it)
...
#endif
				{
					...
					info_2_json(it->second,output_str,this);
					os << output_str << "\n";  
				}
		}
		os.close();  
...
bool Search_Engine_Fast::load_from_file()
{
	std::ifstream is;  
...
//
bool Search_Engine_Fast::search_find_first_term(int search_mode,
		...
		std::vector<bool> & term_vec_if_skip)
{
	...
	if (search_mode == and_mode)
	{
		...
		{
			index_hash_value i_hash_value = _index_core.find_index(query_term(in_term_list[i]));
...
void Search_Engine_Fast::search_recall(int search_mode,
		std::vector<std::string> & in_term_list,
		int term_pos,
		std::vector<bool> & term_vec_if_skip,
...
{
	_index_core.all_query_index(query_term(in_term_list[term_pos]),query_in_it);
	...
	if (search_mode == and_mode)
	{
		for (int i = 0;i < in_term_list.size(); i++)
		{
			...
			_index_core.cross_query_index(query_term(in_term_list[i]),query_in_it,query_out_it);
	...
	else
	{
		for (int i = 0;i < in_term_list.size(); i++)
		{
			...
			_index_core.all_query_index(query_term(in_term_list[i]),tmp_vec);
...
void Search_Engine_Fast::search_compute(std::vector<uint32_t> & query_in_it,
		std::string & in_query,
		std::vector<std::string> & in_term_list,
		Json::Value & query_json,
		int search_mode,
		std::vector< sort_myclass > & vect_score)
{
	...
	//cook term_list_int for search fast
	std::vector<uint32_t> in_term_list_int;
	for (int i = 0;i < in_term_list.size();i++)
...
void Search_Engine_Fast::search_filter(std::vector<std::string> & in_term_list,
		std::string & in_query,
		std::vector<info_storage>  & out_info_vec_ori,
		int search_mode,
		std::vector<bool> &out_ret_vec_ori)
