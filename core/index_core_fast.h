...
#ifndef __INDEX_CORE_FAST_H_
#define __INDEX_CORE_FAST_H_
...
class Index_Core_Fast
{
	public:
		Index_Core_Fast(int data_num = 512,int inner_lock_num = 128):_data_num((uint32_t)data_num)
									...
									,_hash_now_num(0),_shrink_buffer_now(0)
	{
		...
		//index function
		void all_query_index(uint32_t index_hash_key,std::vector<uint32_t> &query_out);
		//the both vector(query_in vector and key -> vector),if query_in size = 0,return key -> vector
		void cross_query_index(uint32_t index_hash_key,std::vector<uint32_t> &query_in,std::vector<uint32_t> &query_out);
		//check if insert_in is bigger than the last data
		bool insert_index(uint32_t index_hash_key,uint32_t insert_in_value);
		bool delete_index(uint32_t index_hash_key,uint32_t delete_in_value);
		bool clear_index(uint32_t index_hash_key);
		bool shrink_index(uint32_t index_hash_key,bool adapt_mem = true);
		void clear_all_index();

		//find index info
		index_hash_value find_index(uint32_t index_hash_key);

	private:
#ifdef _USE_HASH_
		std::tr1::unordered_map<uint32_t,index_hash_value> index_map;
#else
		std::map<uint32_t,index_hash_value> index_map;
#endif
		uint32_t _data_num;
